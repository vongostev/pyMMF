...
def findPropagationConstants(wl, indexProfile, tol=1e-9):
    ...
    while len(roots):
        ...
        def froot(x0): return root(root_func, x0, tol=tol)
        ...
        roots = np.unique(
            [np.round(r/tol)*tol for r in roots if (r > 0 and r < v)]).tolist()
        roots_num = len(roots)

        if roots_num:
            degeneracy = 1 if m == 0 else 2
            modes.betas.extend([np.sqrt((2*np.pi/lbda*n1)**2-(r/a)**2)
                               for r in roots]*degeneracy)
    ...
    logger.info("Found %g modes in %0.2f seconds." %
...
def associateLPModeProfiles(modes, indexProfile, degenerate_mode='sin',
    ...
    if degenerate_mode == 'exp':
        modes.m = \
            [m if not (m, l) in zip(modes.m[:idx], modes.l[:idx]) else -
             m for idx, (m, l) in enumerate(zip(modes.m, modes.l))]
