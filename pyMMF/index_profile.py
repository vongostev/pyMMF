...
class IndexProfile():
    ...
    def initParabolicGRIN(self, n1, a, NA):
        ...
        def radialFunc(r): return np.sqrt(
            n1**2.*(1.-2.*(r/a)**2*Delta)) if r < a else n2
    ...
    def initStepIndex(self, n1, a, NA):
        ...
        def radialFunc(r): return n1 if r < a else n2
    ...
    def initStepIndexMultiCoreHex(
        ...
        hex_centers, _ = create_hex_grid(nx=np.round(n) + 5,
                                         ny=np.round(n) + 5,
                                         crop_circ=n / 2,
                                         )
        ...
        try:
            central_core_hex_centers, _ = create_hex_grid(nx=np.round(n) + 5,
                                                          ny=np.round(n) + 5,
                                                          crop_circ=n / 2,
                                                          )
        except:
            central_core_hex_centers = [[0, 0]]
        mask = reduce(
            lambda x, y: x | y,
            [
                np.array([np.all(np.allclose(x, y)) for x in hex_centers])
        ...
        def hexagon(pos):
            ...
            s = cladding_radius
            x, y = np.abs(pos)
            if y < 3**0.5 * min(s - x, s / 2):
                return pos
            return [0, 0]

        cores_coords = (np.apply_along_axis(
            hexagon, 1, (2 * a + core_pitch) * hex_centers) /
    ...
    def initPhotonicCrystalRadial(
        ...
        delta = 1 - n1
        n2 = np.sqrt(n1**2-NA**2)
        self.initStepIndexMultiCoreRadial(
    ...
    def initPhotonicCrystalHex(
            ...
            central_core_radius: float = 0,
            central_core_n1: float = 1):
        delta = 1 - n1
        n2 = np.sqrt(n1**2-NA**2)
        self.initStepIndexMultiCoreHex(
            1, a, delta, NA, core_pitch,
            central_core_radius, central_core_n1,
            pcf_radius)
        self.type = 'PCF'
        self._crop_cladding(n2, cladding_radius)
